		...
		if (orderC == CblasRowMajor ){
			if (TransB == CblasNoTrans){
				for (int colA=0; colA < K; colA++){
					...
					for (int pos = begin; pos < end; pos++){
						_caffe_cpu_axpy(N, A[pos]* alpha, B + (colA * N), 1, C + (indices[pos] * N), 1);
					}
				}
			}else{
				for (int colA=0; colA < K; colA++){
					...
					const int end = ptr[colA+1];
					for (int pos = begin; pos < end; pos++){
						_caffe_cpu_axpy(N, A[pos]* alpha, B + colA, K, C + (indices[pos] * N), 1);
					}
				}
			}
		}else{
			if (TransB == CblasNoTrans){
				for (int colA=0; colA < K; colA++){
					...
					const int end = ptr[colA+1];
					for (int pos = begin; pos < end; pos++){
						_caffe_cpu_axpy(N, A[pos]* alpha, B + (colA * N), 1, C + indices[pos] , M);
					}
				}
				...
				for (int colA=0; colA < K; colA++){
					...
					const int end = ptr[colA+1];
					for (int pos = begin; pos < end; pos++){
						_caffe_cpu_axpy(N, A[pos]* alpha, B + colA, K, C + indices[pos] , M);
					}
				}
			}
		}
