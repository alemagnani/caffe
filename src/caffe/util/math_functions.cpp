		...
		if (orderC == CblasRowMajor ){
			if (TransB == CblasNoTrans){
				for (int rowA=0; rowA < M; rowA++){
					...
					for (int pos = begin; pos < end; pos++){
						...
						_caffe_cpu_axpy(N, AatPos, BcolAN, 1, Crow_tmp, 1);
						/*
						for (int colC=0; colC < N; colC++){
							Crow_tmp[colC] += AatPos * BcolAN[colC];
						}
						*/
					}
					Dtype*  CrowA = C + (N * rowA);
					_caffe_cpu_axpy(N, alpha, Crow_tmp, 1, CrowA, 1);
					/*
					for (int colC=0; colC < N; colC++){
						CrowA[colC] +=  alpha * Crow_tmp[colC];
					}
					*/
				}
			}else{
				for (int rowA=0; rowA < M; rowA++){
					...
					for (int pos = begin; pos < end; pos++){
						...
						_caffe_cpu_axpy(N, AatPos, BcolA, K, Crow_tmp, 1);
						/*
						for (int colC=0; colC < N; colC++){
							Crow_tmp[colC] += AatPos * BcolA[colC * K];
						}
						*/
					}
					Dtype*  CrowA = C + (N * rowA);
			...
			if (TransB == CblasNoTrans){
				for (int rowA=0; rowA < M; rowA++){
					...
					Dtype*  CrowA = C + rowA;
					_caffe_cpu_axpy(N, alpha, Crow_tmp, 1, CrowA, M);
					/*
					for (int colC=0; colC < N; colC++){
						const int offsetC = colC * M;
						CrowA[offsetC] = CrowA[offsetC] + alpha * Crow_tmp[colC];
					}
					*/
				}
			}else{
		...
		if (orderC == CblasRowMajor ){
			if (TransB == CblasNoTrans){
				for (int colA=0; colA < K; colA++){
					...
					for (int pos = begin; pos < end; pos++){
						_caffe_cpu_axpy(N, A[pos] * alpha, BColAN , 1, C + (indices[pos] * N), 1);
					}
				}
			}else{
				for (int colA=0; colA < K; colA++){
					...
					for (int pos = begin; pos < end; pos++){
						_caffe_cpu_axpy(N, A[pos]* alpha, BColA, K, C + (indices[pos] * N), 1);
					}
				}
			}
		}else{
			if (TransB == CblasNoTrans){
				for (int colA=0; colA < K; colA++){
					...
					const Dtype* BColAN = B + (colA * N);
					for (int pos = begin; pos < end; pos++){
						_caffe_cpu_axpy(N, A[pos]* alpha, BColAN, 1, C + indices[pos] , M);
					}
				}
				...
				for (int colA=0; colA < K; colA++){
					...
					const Dtype* BColA = B + colA;
					for (int pos = begin; pos < end; pos++){
						_caffe_cpu_axpy(N, A[pos]* alpha, BColA, K, C + indices[pos] , M);
					}
				}
			}
		}
