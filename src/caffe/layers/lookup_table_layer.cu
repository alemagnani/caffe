  ...
  IntegerBlob<Dtype> * bottomIntegerBlob =
    ...
    for (int i = 0; i < NUM_; i++) {
      for (int w = 0; w < INPUT_SIZE_; w++) {
        ...
        caffe_gpu_axpy(SIZE_, (Dtype) 1., weight + pos * SIZE_, top_data +( i * (INPUT_SIZE_ * SIZE_) + w), 1, INPUT_SIZE_);
      }
    }
}
  ...
  if (this->param_propagate_down_[0]) {
      ...
      for (int i = 0; i < NUM_; i++) {
        for (int w = 0; w < INPUT_SIZE_; w++) {
          ...
          CHECK_GE(pos, 0);
          CHECK_LT(pos, N_INDEX_);
          caffe_gpu_axpy(SIZE_, (Dtype) 1., top_diff +( i * (INPUT_SIZE_ * SIZE_) + w), diff + pos * SIZE_, INPUT_SIZE_, 1);
        }
      }
    }
    if (propagate_down[0]) {
