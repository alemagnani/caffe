...
void InnerProductLayer<Dtype>::Forward_sparse_cpu(
  ...
  caffe_cpu_csr_gemm<Dtype>(CblasNoTrans, CblasTrans, this->M_, this->N_,
...
template<typename Dtype>
void InnerProductLayer<Dtype>::Backward_sparse_cpu(
  ...
  if (this->param_propagate_down_[0]) {
    ...
    const int* bottom_ptr = bottomSparseBlob->cpu_ptr();
    const int nzz = bottomSparseBlob->nzz();
    caffe_cpu_csr_gemm<Dtype>(CblasTrans, CblasNoTrans, this->K_, this->N_,
