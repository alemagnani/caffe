...
: filename_(new string(tmpnam(NULL))),
  ...
  blob_top_label_(new Blob<Dtype>()) {}
	...
	void TestRead() {
		...
		DataSparseInputParameter* data_param = param.mutable_data_sparse_input_param();
		...
		for (int iter = 0; iter < 100; ++iter) {
			...
			for (int i = 0; i < 6; ++i) {
				EXPECT_EQ( (i+1) * (i+2)/2,blob_top_data_->cpu_ptr()[i+1]) << "debug ptr: iter " << iter << " i " << i ;
				for (int j = 0; j < i; ++j) {
					EXPECT_EQ(j+1, blob_top_data_->cpu_data()[blob_top_data_->cpu_ptr()[i]+j])
            				  << "debug data: iter " << iter << " i " << i << " j " << j;
					EXPECT_EQ(j, blob_top_data_->cpu_indices()[blob_top_data_->cpu_ptr()[i]+j])
	...
	void TestRead2() {
		LayerParameter param;
		DataSparseInputParameter* data_param = param.mutable_data_sparse_input_param();
		data_param->set_batch_size(3); //half the previous batch size to alternate between 2 different dataset
		data_param->set_source(filename_->c_str());
		DataLayerSparseInput<Dtype> layer(param);
		layer.SetUp(blob_bottom_vec_, &blob_top_vec_);
	...
	virtual ~DataLayerSparseTest() { delete blob_top_data_; delete blob_top_label_; }
